...
static const char* FS_FULLSCREEN =
    ...
    "\n"
    "    // ── UV-distortion effects (audio is sole controller) ──\n"
    "    if (uAudioLevel > 0.001) {\n"
    "        if (uAudioEffect == 0) {\n"
    "            // Liquid Ripple: standing waves, amplitude ∝ audio\n"
    "            float wave = sin(uv.y * 30.0) * cos(uv.x * 20.0);\n"
    "            uv += wave * uAudioLevel * 0.03;\n"
    "        } else if (uAudioEffect == 1) {\n"
    "            // Cyber Glitch: scanline tear, position & count ∝ audio\n"
    "            float tear = step(1.0 - uAudioLevel * 0.6, fract(uv.y * 40.0 + uAudioLevel * 35.0));\n"
    "            uv.x += tear * (uAudioLevel - 0.3) * 0.12;\n"
    "        } else if (uAudioEffect == 3) {\n"
    "            // Bass Pixelate: cell size ∝ audio (loud = coarser)\n"
    "            float cells = mix(800.0, 12.0, uAudioLevel * uAudioLevel);\n"
    "            uv = floor(uv * cells) / cells;\n"
    "        } else if (uAudioEffect == 4) {\n"
    "            // Audio Zoom: zoom amount ∝ audio\n"
    "            uv = (uv - 0.5) * (1.0 - uAudioLevel * 0.25) + 0.5;\n"
    "        } else if (uAudioEffect == 6) {\n"
    "            // Fisheye/Barrel: bulge strength ∝ audio squared (subtle at low, dramatic at peak)\n"
    "            vec2 p = uv - 0.5;\n"
    "            float d = dot(p, p);\n"
    "            uv = uv + p * d * uAudioLevel * uAudioLevel * 5.0;\n"
    "        } else if (uAudioEffect == 7) {\n"
    "            // Radial Ring: ring radius IS the audio level — sweeps outward as beat rises\n"
    "            vec2 p = vUV - 0.5;\n"
    "            p.x *= uAspect;\n"
    "            float dist = length(p);\n"
    "            float ringR = uAudioLevel * 0.75;\n"
    "            float ringW = 0.03 + uAudioLevel * 0.03;\n"
    "            float shock = exp(-pow((dist - ringR) / ringW, 2.0)) * uAudioLevel;\n"
    "            vec2 dir = normalize(vUV - 0.5 + vec2(0.0001));\n"
    "            uv += dir * shock * 0.07;\n"
    "        } else if (uAudioEffect == 8) {\n"
    "            // Audio Shockwave: Pure audio-driven ripple\n"
    "            vec2 p = vUV - 0.5;\n"
    "            p.x *= uAspect;\n"
    "            float dist = length(p);\n"
    "            // The phase of the ripple pushes outward instantly with volume\n"
    "            float wave = sin(dist * 40.0 - uAudioLevel * 20.0);\n"
    "            // Distort only when there is audio, stronger when louder\n"
    "            float strength = exp(-dist * 2.0) * uAudioLevel * 0.15;\n"
    "            vec2 dir = normalize(vUV - 0.5 + vec2(0.0001));\n"
    "            uv += dir * wave * strength;\n"
    "        }\n"
    "    }\n"
    "\n"
    ...
    "    }\n"
    "\n"
    "    // ── Sample texture (Chromatic Aberration needs special path) ──\n"
    "    vec4 c = vec4(0.0);\n"
    "    if (uAudioLevel > 0.001 && uAudioEffect == 5) {\n"
    "        // Chromatic Aberration: R/G/B sampled at different UV offsets ∝ audio\n"
    "        float shift = uAudioLevel * 0.035;\n"
    "        vec2 dir = normalize(vUV - 0.5 + vec2(0.0001));\n"
    "        if (uIsNV12 == 1) {\n"
    "            float y_r = texture(uTexY, clamp(uv + dir * shift, 0.0, 1.0)).r - (16.0/255.0);\n"
    "            float y_g = texture(uTexY, uv).r - (16.0/255.0);\n"
    "            float y_b = texture(uTexY, clamp(uv - dir * shift, 0.0, 1.0)).r - (16.0/255.0);\n"
    "            float u2 = texture(uTexUV, uv).r - 0.5;\n"
    "            float v2 = texture(uTexUV, uv).g - 0.5;\n"
    "            c.r = clamp(1.16438*y_r + 1.79274*v2, 0.0, 1.0);\n"
    "            c.g = clamp(1.16438*y_g - 0.21325*u2 - 0.53291*v2, 0.0, 1.0);\n"
    "            c.b = clamp(1.16438*y_b + 2.11240*u2, 0.0, 1.0);\n"
    "            c.a = 1.0;\n"
    "        } else {\n"
    "            c.r = texture(uTexY, clamp(uv + dir * shift, 0.0, 1.0)).r;\n"
    "            c.g = texture(uTexY, uv).g;\n"
    "            c.b = texture(uTexY, clamp(uv - dir * shift, 0.0, 1.0)).b;\n"
    "            c.a = texture(uTexY, uv).a;\n"
    "        }\n"
    "    } else if (uIsNV12 == 1) {\n"
    "        float y = texture(uTexY, uv).r - (16.0 / 255.0);\n"
    "        float u2 = texture(uTexUV, uv).r - 0.5;\n"
    "        float v2 = texture(uTexUV, uv).g - 0.5;\n"
    "        float r = 1.16438 * y + 1.79274 * v2;\n"
    "        float g = 1.16438 * y - 0.21325 * u2 - 0.53291 * v2;\n"
    "        float b = 1.16438 * y + 2.11240 * u2;\n"
    "        c = vec4(r, g, b, 1.0);\n"
    "    } else {\n"
    "        c = texture(uTexY, uv);\n"
    "    }\n"
    "\n"
    "    // ── Color-space effects (audio is sole controller) ──\n"
    "    if (uAudioLevel > 0.001) {\n"
    "        if (uAudioEffect == 0) {\n"
    "            // Liquid Ripple: saturation boost ∝ audio\n"
    "            float lum = dot(c.rgb, vec3(0.299, 0.587, 0.114));\n"
    "            c.rgb = mix(vec3(lum), c.rgb, 1.0 + uAudioLevel * 1.5);\n"
    "            c.rgb *= 1.0 + uAudioLevel * 0.4;\n"
    "        } else if (uAudioEffect == 1) {\n"
    "            // Cyber Glitch: channel cycle ∝ audio magnitude\n"
    "            c.rgb = mix(c.rgb, c.brg, uAudioLevel * uAudioLevel);\n"
    "        } else if (uAudioEffect == 2) {\n"
    "            // Cinematic Heartbeat: vignette darkness ∝ audio\n"
    "            vec2 pv = vUV - 0.5;\n"
    "            pv.x *= uAspect;\n"
    "            float dist = length(pv);\n"
    "            float vig = smoothstep(0.8, 0.1, dist * (1.0 + uAudioLevel));\n"
    "            c.rgb *= vig + uAudioLevel * 0.8;\n"
    "        } else if (uAudioEffect == 7) {\n"
    "            // Radial Ring: bright ring where ring position = audio\n"
    "            vec2 pr = vUV - 0.5;\n"
    "            pr.x *= uAspect;\n"
    "            float dist = length(pr);\n"
    "            float ringR = uAudioLevel * 0.75;\n"
    "            float glow = exp(-pow((dist - ringR) / 0.04, 2.0)) * uAudioLevel * 2.5;\n"
    "            c.rgb = clamp(c.rgb + glow, 0.0, 2.0);\n"
    "        }\n"
    "    }\n"
    "\n"
